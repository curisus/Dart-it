...
파일 없이 데이터만 보려면 원격 서버와 동일한 `list_report_sections`·`get_report_sections`·`get_financial_statements`·`get_major_accounts`·`get_financial_indicators`·`get_report_topics`·`get_company_profile`·`get_ownership_reports`·`get_material_events`·`get_registration_statements`(아래 원격 도구 사용 순서 참조)도 로컬에서 그대로 사용할 수 있습니다. 로컬 서버는 전체 기능을 제공하며 API 키가 컴퓨터 밖으로 나가지 않고, 원격 서버는 설치 없이 모든 조회 도구를 제공합니다(파일 반출은 로컬 전용).
...
- 도구: `search_companies`, `list_report_filings`, `list_report_attachments`, `list_report_sections`, `get_report_sections`, `get_financial_statements`, `get_major_accounts`, `get_financial_indicators`, `get_report_topics`, `get_company_profile`, `get_ownership_reports`, `get_material_events`, `get_registration_statements` 13개 (조회 전용)
...
8. `get_material_events(corp_code, event_types, bgn_de, end_de)`로 주요사항보고(DS005) 36종을 조회합니다. `get_ownership_reports`와 달리 `bgn_de`·`end_de`(`YYYYMMDD`)는 필수입니다(DART DS005 API 자체가 접수일 기간을 요구). `event_types`에는 부도발생·영업정지·증자/감자 결정·전환사채 등 발행결정·자기주식 취득/처분·합병/분할·해외상장 등 36가지 사유 키 중 최대 10개까지 지정할 수 있으며, 전체 키 목록은 지원하지 않는 `event_type`으로 호출했을 때 오류 상세(`supported_event_types`)에 반환됩니다. 결과는 요청한 `event_types` 순서대로 반환되며, 유형별 행 수(`row_count`)와 원문 필드를 그대로 담습니다. 해당 기간에 요청한 유형의 공시가 없는 경우도(요청한 유형 전부가 비어 있더라도) 실패가 아니라 빈 목록과 경고로 응답합니다 — 그 기간에 해당 사유의 공시가 없었다는 사실 자체가 유효한 답이기 때문입니다.
9. `get_registration_statements(corp_code, stmt_type, bgn_de, end_de)`로 증권신고서 주요정보(DS006)를 조회합니다. `bgn_de`·`end_de`(`YYYYMMDD`)는 필수이며, DART 접수일이 이 기간 안에 있는 신고서를 찾습니다. `stmt_type`은 한 번에 하나만 지정하며 `equity_securities`(지분증권), `debt_securities`(채무증권), `depositary_receipts`(증권예탁증권), `merger`(합병), `stock_exchange_transfer`(주식의 포괄적 교환·이전), `division`(분할) 중 하나입니다. 결과는 DART가 내려주는 공식 그룹 제목별로 묶이고, 각 그룹은 `row_count`와 원문 필드 그대로의 `rows`를 담습니다. 해당 기간에 자료가 없으면 실패가 아니라 빈 그룹/빈 행 수와 경고로 응답합니다.
