...
| 표면 | 컨셉 | 도구 구성 |
|---|---|---|
| **로컬** (`mcp_server.py`, stdio) | 전체 기능 superset. API 키가 컴퓨터 밖으로 나가지 않는 안전성이 강점. 파일 반출(xlsx, 향후 md 등)을 포함해 더 넓은 작업을 지원 | 공통 조회 8종 + export 그룹 |
| **원격** (`remote_server.py`, Vercel) | 모든 조회 도구를 제공. 설치 없이 사용, 키는 요청마다 전달(헤더 > Bearer > `?key=`) | 공통 조회 8종 전부 |
